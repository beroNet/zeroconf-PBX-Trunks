...
//    echo "<input type=button value=".et('Refresh')." onclick=\"atualizar()\"></tr></form>";
    ...
    //else
    echo "<a href=\"index.php\" target=\"_top\">Back</a>";
    $aux=$fm_root_atual;
...
function dir_list_form() {
    ...
    if ($opdir = @opendir($dir_atual)) {
        ...
        if ($dir_atual == $fm_root_atual) {
                echo "<br><br><center><b><font size=16>Welcome to the Asterisk configuration</font></b></center>";
                return;
        }
        ...
        function edit_file(arg){
                var w = 800;
                var h = 600;
                window.open('".$path_info["basename"]."?action=7&dir_atual=$dir_atual&filename='+escape(arg), '', 'width='+w+',height='+h+',fullscreen=no,scrollbars=no,resizable=yes,status=no,toolbar=no,menubar=no,location=no');
        }
        function config(){
                var w = 600;
                var h = 400;
            ...
            foreach ($entry_list as $ind=>$dir_entry) {
                 ...
                 if ($dir_entry["type"]=="dir"){
                     ...
                     if ( is_readable($dir_atual.$file) && (strpos(".wav#.mp3#.mid#.avi#.mov#.mpeg#.mpg#.rm#.iso#.bin#.img#.dll#.psd#.fla#.swf#.class#.ppt#.jpg#.gif#.png#.wmf#.eps#.bmp#.msi#.exe#.com#.rar#.tar#.zip#.bz2#.tbz2#.bz#.tbz#.bzip#.gzip#.gz#.tgz#", $dir_entry["ext"]."#" ) === false)) $file_out .= "
                                 <td bgcolor=\"#00FFFF\" align=center><a href=\"javascript:edit_file('$file')\">".et('Edit')."</a>";
